...
public class DateUtil {
     ...
     * @author:
     * @修改记录： ==============================================================<br>
     * 日期:2004-11-4  创建方法，并实现功能
     * ==============================================================<br>
     */
    public static Date strToDate(final String strn) {
        Date returnValue = null;

        if (strn != null && !strn.equals("") && !strn.equalsIgnoreCase("null")) {
/*            final DateFormat df = DateFormat.getDateInstance();
     ...
     * Date())返回的时间为2004-10-16 2004-11-4  修正月份相减时(12月份、月份天数不一致时)问题.<br>
     *
     ...
     * @author:
     * @修改记录： ==============================================================<br>
     * 日期:2004-11-04  创建方法，并实现其功能
     * ==============================================================<br>
     */
    public static Date dateAdd(final String timeinterval, final int number, final Date sd) {
     ...
     * @author:
     * @修改记录： ==============================================================<br>
     * 日期:2007-9-4  创建方法，并实现其功能
     * ==============================================================<br>
     */

    public static Date dateTimeAdd(final String timeinterval, final int number, final Date sd) {

        return dateAdd(timeinterval, number, sd);
      ...
      /*  Date returnDate = null;
        ...
        lpMinutes = Integer.parseInt(DateUtil.getMinutes(DateUtil.dateTimeToStr(sd)));
        lpSeconds = Integer.parseInt(DateUtil.getSeconds(DateUtil.dateTimeToStr(sd)));
        ...
        if (timeinterval.toLowerCase().equals("y")) {
            ...
            int internal = 0;
            if (number > 0) {
                lpStrDate = StrUtil.strnull(lpYear) + "-" + StrUtil.strnull(lpMonth) + "-" + "1";
                ...
                lpStrDate = StrUtil.strnull(lpYear) + "-" + StrUtil.strnull(lpMonth) + "-" + "1";
                lpsd = DateUtil.strToDate(lpStrDate);
                lpStrDate = DateUtil.getYear(sd) + "-" + DateUtil.getMonth(sd) + "-" + "1";
                lped = DateUtil.strToDate(lpStrDate);
                internal = DateUtil.dateDiff("d", lpsd, lped);
            }
            if (lpDay > internal) {
     ...
     * @author:
     * @修改记录： ==============================================================<br>
     * 日期:2004-10-08  创建方法，并实现功能
     * ==============================================================<br>
     */
    public static int dateDiff(final String timeinterval, final Date sd, final Date ed) {
        int returnValue = 0;

        if (timeinterval.toLowerCase().equals("m")) {
            final int lpyear = Integer.parseInt(DateUtil.getYear(DateUtil.dateToStr(ed)))
                    - Integer.parseInt(DateUtil.getYear(DateUtil.dateToStr(sd)));
            if (lpyear >= 0) {
                final int ret = lpyear * 12;
                final int edmomth = ret
                        + Integer.parseInt(DateUtil.getMonth(DateUtil.dateToStr(ed)));
                returnValue = edmomth - Integer.parseInt(DateUtil.getMonth(DateUtil.dateToStr(sd)));
            }
        } else if (timeinterval.toLowerCase().equals("d")) {
            returnValue = Integer.parseInt(String.valueOf((ed.getTime() - sd.getTime())
                    / (3600 * 24 * 1000)));
        } else if (timeinterval.toLowerCase().equals("s")) { // added by 
            returnValue = Integer.parseInt(String.valueOf((ed.getTime() - sd.getTime()) / 1000));
        }
        return returnValue;
    }
    ...
    public static String dateDiffToString(final Date sd, final Date ed) {
        ...
        iHour = Integer.parseInt(String.valueOf((ed.getTime() - sd.getTime()) / (1000 * 3600)));

        iMinute = Integer.parseInt(String
                .valueOf(((ed.getTime() - sd.getTime()) / (1000 * 60) - iHour * 60)));
    ...
    public static int dateDiffDays(final String toDate, final String fromDate) {
        ...
        g1 = new GregorianCalendar(Integer.parseInt(toDate.substring(0, 4)), Integer
                .parseInt(toDate.substring(5, 7)) - 1, Integer.parseInt(toDate.substring(8)));
        g2 = new GregorianCalendar(Integer.parseInt(fromDate.substring(0, 4)), Integer
                .parseInt(fromDate.substring(5, 7)) - 1, Integer.parseInt(fromDate.substring(8)));
    ...
    public static Timestamp getNowSqlDateTime() {
        ...
        final Timestamp sqlTimestamp = new Timestamp(nCurrentTime);
        return sqlTimestamp;
    }
    ...
    public static java.sql.Time getNowSqlTime() {
        final long nCurrentTime = System.currentTimeMillis();
        final Date utilDate = new Date(nCurrentTime);
...
//            final String hms = DateFormat.getTimeInstance().format(date);
            final String hms = time.format(date);
     ...
     * ==============================================================<br>
     */
    public static String dateStrSerial(final String lpYear, String lpMonth, String lpDay) {
        String lpDateStr = "";

        if (!StrUtil.strnull(lpYear).equals("") && !StrUtil.strnull(lpMonth).equals("")
    ...
    public static Date dateSerial(final String lpYear, final String lpMonth, final String lpDay) {
        String lpDateStr = "";
    ...
    public static String getNowHour(final Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        int hour = calendar.get(Calendar.HOUR_OF_DAY) ;
        if (hour >= 10) {
    ...
    public static String getNextHour(final Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
    ...
    public static String getCustomTimeToStandardTimeStr(final String lpTimeStr) {
        ...
        if (!StrUtil.strnull(lpTimeStr).equals("") && lpTimeStr.length() == 6) {
            lpReturnValue = lpTimeStr.substring(0, 2) + ":" + lpTimeStr.substring(2, 4) + ":"
    ...
    public static String geCustomDateStrtToStandardDateStr(final String strYMD) {
        ...
        if (strYMD == null) {
            ...
            if (strYMD.length() == 8) {
                lpReturnValue = strYMD.substring(0, 4) + "-" + strYMD.substring(4, 6) + "-"
                        + strYMD.substring(6, 8);
            } else {
                lpReturnValue = strYMD;
            }
        }
     ...
     * ==============================================================<br>
     */
    public static Date sql2util(final java.sql.Date aDate) {
        if (aDate == null) {
            return null;
        }
     ...
     * "20030801"。
     *
     * @param ogdate 日期的字符形式，必须是六位（年月）或者八位（年月日）数字； 如："200308"（年月）或者 20030804（年月日）。
     * @return String
     * @author:
     * @修改记录： ==============================================================<br>
     * 日期:2004-1-6  创建方法，并实现其功能
    ...
    public static Calendar getToday() throws ParseException {
        ...
        final SimpleDateFormat df = new SimpleDateFormat(DateUtil.getDatePattern());
    ...
    public static final String convertDateTimeToString2(final Date aDate) {
        return DateUtil.getDateTime("yyyyMMddHHmmss", aDate);
    }

    public static final Date getStartDate() {
        Date aDate = DateUtil.getNowDate();
        String sDate = DateUtil.getDateTime("yyyy-MM-dd", aDate) + " 00:00:00";
        Date convertStringToDate = null;
        try {
            convertStringToDate = DateUtil.convertStringToDate("yyyy-MM-dd HH:mm:ss", sDate);
    ...
    public static final Date getEndDate() {
        Date aDate = DateUtil.getNowDate();
        String sDate = DateUtil.getDateTime("yyyy-MM-dd", aDate) + " 23:59:59";
        Date convertStringToDate = null;
        try {
            convertStringToDate = DateUtil.convertStringToDate("yyyy-MM-dd HH:mm:ss", sDate);
        } catch (ParseException e) {
    ...
    public static String workToDate(final int intervalHour) {
        ...
        if (day0 == 7 || day0 == 1) { // 周末受理的单算是周一零点受理的 crm00008001
            ...
            final String calDateStr = simpledateformat.format(cal.getTime()) + " 00:00:00";
            try {
    ...
    public static void main(final String[] args) {
      ...
      /*  String aa = "yyyyMMddHHmi";
        try {
            DateUtil.convertStringToDate("yyyyMMddHHmi", "201411052053");
        } catch (ParseException e) {
            System.out.println("77777777777");
            e.printStackTrace();
        }*/
      /*  final String ss = DateUtil.workToDate(25);
        ...
        System.out.println(isDate("2014/10/33", "yyyy/MM/dd"));
        ;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");
        try {
            Date date = sdf.parse("2014/10/30");
     ...
     * @author:
     * @修改记录： ==============================================================<br>
     * 日期:2008-5-25  创建方法，并实现其功能
     * ==============================================================<br>
     */
    public static int dateDiffweekDay(Date d1, Date d2) {
        int count = 0;
        count = Integer
                .parseInt(String.valueOf((d2.getTime() - d1.getTime()) / (1000 * 3600 * 24)));
        d1 = DateUtil.strToDate(DateUtil.getDate(d1));
        d2 = DateUtil.strToDate(DateUtil.getDate(d2));
        Date d = (d1.before(d2) ? d1
                : d2);
    ...
    public static String datetime2customizestr(final Date date) {
        ...
        if (datetime != null && datetime.length() == 19) {
            str = datetime.substring(0, 4) + datetime.substring(5, 7) + datetime.substring(8, 10)
     ...
     * ==============================================================<br>
     */
    public static Date dateTimeAddIgnoreStartEnd(final int number, final Date sd,
        ...
        if (endTime.compareTo(startTime) < 0
            ...
            endDate = DateUtil.dateTimeToStr(DateUtil.dateTimeAdd("d", 1, DateUtil
                    .strToDateTime(todayTime.substring(0, 11).concat(endTime))));
        }
        // 开始日期大于结束日期，则开始日期要减一
        if (startDate.compareTo(endDate) > 0) {
            startDate = DateUtil.dateTimeToStr(DateUtil.dateTimeAdd("d", -1, DateUtil
                    .strToDateTime(startDate)));
        }
        ...
        if (startDate.compareTo(todayTime) < 0 && endDate.compareTo(todayTime) > 0) {
            ...
            tempMinutes = DateUtil.dateDiff("s", DateUtil.strToDateTime(todayTime), DateUtil
                    .strToDateTime(startDate)) / 60;
            ignoreMinutes = DateUtil.dateDiff("s", DateUtil.strToDateTime(startDate), DateUtil
                    .strToDateTime(endDate)) / 60;
        }
     ...
     * @param endTime   10:00
     * @param endTime   format 返回单位 H 小时 M 分钟
     * @return
     */
    public static Double getTimelenNum(String startTime, String endTime, String format) {
    ...
    /**
     * 方法功能: .
     * 格式化时间字符串 如20091209 格式化成 2009-12-09
     *
     * @author 2012-1-14
     * @修改记录： ==============================================================<br>
     * 日期:2012-1-14  创建方法，并实现其功能
     * ==============================================================<br>
     */
    ...
    public static Date getNextMonth(Date date) {
        String ymd = DateUtil.getYear(date) + "-" + DateUtil.getMonth(date) + "-1";
    ...
    public static Date getNextDate(Date date) {
        Date nextDate = DateUtil.dateAdd("d", 1, date);
        return nextDate;
    }
    ...
    public static Date getDayStartTime(Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date); //获得实体对象里面一个Date类型的属性，set进Calender对象中。
        ...
        return calendar.getTime();//然后从Calender对象中得到修改过的属性，重新set进实体中
    }
     ...
     * @return
     */
    public static boolean checkToday(Date beginDate, Date endDate) {
     ...
     * @return Set
     */
    public static Set<String> getBefore7Day(Date date, String format) {
        if (date != null) {
            Set<String> daySet = new LinkedHashSet<String>();
            for (int i = 6; i >= 0; i--) {
                daySet.add(formatDate(dateAdd("d", 0 - i, date), format));
            }
            return daySet;
        }
    ...
    /**
     * 获取12月份的数据 为 01,02,03,04,05....12
     *
     * @return
     */
    public static Set<String> get12MonthDate() {
    ...
    /**
     * 获取输入时间段的数据
     * 返回格式 （输入为7,22）：07,08,,09....22
     *
     * @return
     * @author huagnlx 2018-03-16
     */
    public static Set<String> getDayHh(int beginAxis, int endAxis) {
        Set<String> dayHh = new TreeSet<String>();
        for (int i = beginAxis; i <= endAxis; i++) {
            dayHh.add(String.format("%02d:00", i));
        }
        return dayHh;
    }
